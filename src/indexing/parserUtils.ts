...
export function pythonizeType(rawType: string | undefined): string {
  ...
  // Rust containers occur in the same IR as TypeScript annotations when the
  // tree-sitter Rust parser feeds the shared Python skeleton emitter.
  value = value.replace(/\bVecDeque<([^>]+)>/g, 'list[$1]')
  value = value.replace(/\bVec<([^>]+)>/g, 'list[$1]')
  value = value.replace(/\bHashSet<([^>]+)>/g, 'set[$1]')
  value = value.replace(/\bBTreeSet<([^>]+)>/g, 'set[$1]')
  value = value.replace(/\b(?:HashMap|BTreeMap)<([^,>]+),\s*([^>]+)>/g, 'dict[$1, $2]')
  value = value.replace(/\bOption<([^>]+)>/g, '$1 | None')
  value = value.replace(/\bResult<([^,>]+)(?:,[^>]+)?>/g, '$1')
  value = value.replace(/\b(?:Box|Arc|Rc|Pin|Cow)<([^>]+)>/g, '$1')
  value = value.replace(/\b(?:String|OsString)\b/g, 'str')
  value = value.replace(/\b(?:usize|isize|u8|u16|u32|u64|u128|i8|i16|i32|i64|i128)\b/g, 'int')
  value = value.replace(/\b(?:f32|f64)\b/g, 'float')
...
function parseSingleParameter(
  ...
  const normalizedNamePart = namePart
    .replace(/^(?:&\s*|mut\s+|ref\s+|const\s+)+/g, '')
    .trim()

  if (
    normalizedNamePart === 'this' ||
    normalizedNamePart === 'self' ||
    normalizedNamePart === 'cls'
  ) {
    return null
  }

  let name = normalizedNamePart
